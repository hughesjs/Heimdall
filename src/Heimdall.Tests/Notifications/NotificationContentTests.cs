...
public class NotificationContentTests
{
    ...
    {
        var (title, _) = NotificationContent.Format(new NotificationPayload(TransitionKind.Broke, Run(RunStatus.Failure)));
    ...
    {
        var (title, _) = NotificationContent.Format(new NotificationPayload(TransitionKind.Recovered, Run(RunStatus.Success)));
    ...
    {
        var (_, body) = NotificationContent.Format(new NotificationPayload(TransitionKind.Broke, Run(RunStatus.Failure, branch: "main", actor: "alice")));
    ...
    public void Body_prefers_the_pull_request_number_when_present()
    {
        ...
        var (_, body) = NotificationContent.Format(new NotificationPayload(TransitionKind.Broke, run));
